...
if __name__ in '__main__':
    ...
    logger.info(
        ...
        'we choose a cutoff index of {cutoff_index}. Total wl axis size is {wl.size}')
    logger.info(
        f'For a long-wave cutoff of {CHI2_WL}, '
        'we choose a cutoff index of {long_cutoff_index}. Total wl axis size is {wl.size}')
    ...
    with figure_context(figsize=(12, 12)) as fig:
        ...
        for _axes, numerator in zip(axes, numerators):
            for ax, denom, label, power in zip(_axes, denominators, labels, powers):
                ...
                ax.axvline(CUTOFF_WL.to_value(WL_UNIT), ls='--', c='k')
                ax.axvline(CHI2_WL.to_value(WL_UNIT), ls='--', c='k')
                regions = [_wl < CUTOFF_WL.to_value(WL_UNIT), (_wl >= CUTOFF_WL.to_value(
                    WL_UNIT)) & (_wl < CHI2_WL.to_value(WL_UNIT)), _wl >= CHI2_WL.to_value(WL_UNIT)]
                for reg in regions:
                    Zreg = Z[:, reg]
                    mean = np.mean(Zreg)
                    stdev = np.std(Zreg)
