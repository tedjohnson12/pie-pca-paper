...
BIN_WL = 6
BIN_TIME = 3
FIGWIDTH = COLWIDTH * 2/3
FIGSIZE = (FIGWIDTH, 0.7*FIGWIDTH)
...
CHI2_NOISE_SCALE = (
        np.sqrt([
        3.062339935171614,3.0164397522412667,2.9471240258326286
    ]),
        np.sqrt([
        2.9344707963359737,2.9571702536063524,2.946854993074575
    ]),
        np.sqrt([
        2.9344707963359737,2.9571702536063524,2.946854993074575
    ])
...
def get_residual_and_noise(chi_noise_scale, epsilon, interpolator, get_model):
...
if __name__ in '__main__':
    ...
    for aperture, noise_scales, interpolator_initializer, get_model, planet_params in zip(
        APERTURES, CHI2_NOISE_SCALE, INTERPOLATORS, MODEL_GETTERS, PLANET_PARAMS
    ):
        interpolator = interpolator_initializer()
        pl_true_radius = planet_params.radius.to(u.R_earth)
        for temperature_ratio, label, noise_scale in zip(TEMPERATURE_RATIOS, LABELS, noise_scales):
