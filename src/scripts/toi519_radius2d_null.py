...
MAX_BASIS = None
NOISE_SCALE = 1.0
CHI2_NOISE_SCALE = np.sqrt(12.952999957790148)
THERMAL_SCALE = 1.0
SEED = 33
...
if __name__ in '__main__':
    ...
    def get_residual_and_noise(chi_noise_scale=1.0):
        ...
        _thermal = 0*THERMAL_SCALE*interpolator([1])[0, :, :].T
    ...
    dist_residual = remove_epoch(dist_residual,eclipse_start,eclipse_end)
    dist_noise = remove_epoch(dist_noise, eclipse_start,eclipse_end)
    ...
    for i, rad in tqdm(enumerate(radius_arr),total=radius_arr.size):
        for j, log_eps in enumerate(log_eps_array):
            ...
            grid_residual = remove_epoch(grid_residual, eclipse_start,eclipse_end)
    ...
    with figure_context(figsize=FIGSIZE) as fig:
        ...
        fig.tight_layout()
        fig.savefig(
            paths.figures / f'{PREFIX}_retrieval_red_chi_square_radius.pdf')
